...
function App() {
  ...
  const [getState, setGetState] = useState('Irvine, USA'); //store the location name from input field
  const [state, setState] = useState('Irvine, USA'); //store a copy of the getState

  // API KEY AND URL
  const apiKey = process.env.REACT_APP_API_KEY; //store the API key created in the .env file
  const apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${state}&appid=${apiKey}`; //store the URL that calls the location from getState and also apiKey
  ...
  };  // convery kelvin to celcius to fahrenheit with no decimals

  return (
    <div className="App">
      <header className="d-flex justify-content-center algin-items-center">
